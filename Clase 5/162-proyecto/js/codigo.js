...
fetch("json/leer.php").then(function(e){
        return e.json();
    }).then(function(data){
    // Borra lo anterior que haya
    ...
    if(partido[1] == "blog"){
        ...
        for(
            ...
            if(data.articulos[i].titulo != undefined){
                document.getElementById("articulos").innerHTML += '<a href="?id='+i+'"><article><div class="imagen" style="background:url(\''+data.articulos[i].imagen+'\');background-size:cover;background-position:center center;"></div><time datetime="'+data.articulos[i].fecha+'">'+data.articulos[i].fecha+'</time><h4>'+data.articulos[i].titulo+'</h4><p>'+data.articulos[i].texto+'</p><button>Saber más</button></article></a>'
            }
        }
